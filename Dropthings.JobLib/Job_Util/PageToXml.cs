...
namespace IdxSource
{
    ...
    public class Pagetoxml
    {
        ...
        {
            ILog loger = log4net.LogManager.GetLogger("GenerateXml");
        ...
        public static string GenerateIDX(string srcUrl, string srcReg, XMLNodeMode nodeMode, string keyWord, bool GenSubSameNews)
        {
            ...
            {
                string idxNode = GetIDXValue(m, nodeMode, keyWord);
                idx.Append(idxNode);
            }
        ...
        public static void GenerateXml(string srcUrl, string srcReg, string fileName,XMLNodeMode nodeMode,string keyWord,bool GenSubSameNews)
        {
            ...
            r = new Regex(srcReg, RegexOptions.IgnoreCase | RegexOptions.Compiled | RegexOptions.Multiline);
        ...
        public static string GetIDXValue(Match m, XMLNodeMode nodeMode, string keyWord)
        {
            switch (nodeMode)
            {
                case XMLNodeMode.BaiduNews:
                    return GetBaiduNewsIdxNode(m, keyWord);
                //case XMLNodeMode.SogouNews:
                //    return GetSogouNewsNode(m);
                //case XMLNodeMode.TianyaGoogle:
                //    return GetTianyaGoogleNode(m);
                ...
                //    return GetBaiduNode(m, keyWord);
                default:
                    throw new Exception("GetIDXValue(XMLNodeMode) 未定义");
            }
        }
        ...
        public static string GetBaiduNewsNode(Match m, string keyWord)
        {
            ...
            string title = NoHTML(m.Groups[3].Value);
            xmlNode.Append("<doc_title><![CDATA[").Append(title).Append("]]></doc_title>");
            ...
            string content = NoHTML(m.Groups[6].Value);
            xmlNode.Append("<doc_content><![CDATA[").Append(content).Append("]]></doc_content>");

            ILog loger = log4net.LogManager.GetLogger("GetPlaceByText");
            PlaceTager tager = new PlaceTager();
            SortedList<string, int> places = new SortedList<string,int>();

            try
            {
                places = tager.GetPlaceByText(title);
            }
            catch (Exception ex)
            {
                loger.Error(title + "\r\n" + ex.Message + "\r\n" + ex.StackTrace);
            ...
            if (places.Keys.Count == 0)
            {
                try
                {
                    places = tager.GetPlaceByText(content);
                }
                catch (Exception ex)
                {
                    loger.Error(content + "\r\n" + ex.Message + "\r\n" + ex.StackTrace);
            ...
            if (places.Keys.Count > 0)
            {
                ...
                {
                    xmlNode.Append("<doc_place><![CDATA[").Append(place).Append("]]></doc_place>");
                }
            }
        ...
        public static string GetBaiduNewsIdxNode(Match m, string keyWord)
        {
            StringBuilder idxNode = new StringBuilder();

            idxNode.Append("#DREREFERENCE ").Append(m.Groups[2].Value).Append("\r\n");
            idxNode.Append("#DRETITLE ").Append(NoHTML(m.Groups[3].Value)).Append("\r\n");
            //idxNode.Append("#DRESECTION 0").Append("\r\n");
            idxNode.Append("#DREDATE ").Append(NoHTML(m.Groups[5].Value)).Append("\r\n");
            idxNode.Append("#DREFIELD myDate=\"").Append(NoHTML(m.Groups[5].Value)).Append("\"\r\n");
            idxNode.Append("#DREFIELD mySiteName=\"").Append(NoHTML(m.Groups[4].Value)).Append("\"\r\n");
            idxNode.Append("#DREFIELD mySrcType=\"news\"\r\n");
            idxNode.Append("#DREFIELD myKeyword=\"").Append(keyWord).Append("\"\r\n");
            idxNode.Append("#DRECONTENT").Append("\r\n").Append(NoHTML(m.Groups[6].Value)).Append("\r\n");
            idxNode.Append("#DREENDDOC\r\n");

            return idxNode.ToString();
