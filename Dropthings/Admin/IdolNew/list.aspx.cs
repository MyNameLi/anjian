...
public partial class Admin_IdolNew_list : System.Web.UI.Page
{    
            ...
            try
            {
                switch (act)
                {
                    ...
                    case "innitsitecolumn":
                        ...
                        IList<COLUMNDEFEntity> columnlist = ColumnFacade.GetListBySiteId(Convert.ToInt32(siteid), 91);
                        StringBuilder optionstr = new StringBuilder();
                        GetSelectHtml(columnlist, 91, 0, ref optionstr);
                        res = "{\"optionstr\":\"" + EncodeByEscape.GetEscapeStr(optionstr.ToString()) + "\",\"Success\":1}";
                        break;
                    case "innitcategory":
                        int parentcate = Convert.ToInt32(Paramas["parentcate"]);
                        IList<CATEGORYEntity> categorylist = CategoryFacade.GetCategoryEntityList(" ID!=691");
                    ...
                    case "newsdelete":
                        ...
                        StringBuilder sql_posturl = new StringBuilder();
                        foreach (string l_url in l_urllist)
                        {
                            if (sql_posturl.Length > 0)
                            {
                                sql_posturl.Append(",");
                            }
                            sql_posturl.Append("'").Append(l_url).Append("'");
                        }
                        //ArchiveInfoFacade.deleteByUrl(sql_posturl.ToString(), 1);
                        ArticleFacade.DeleteByUrl(sql_posturl.ToString(), 91);
                        ChangeDataBase(l_posturl.ToString());
                        res = "{\"Success\":1}";
                        break;
                    case "getcolumnname":
                        string articleurllist = Paramas["url_list"];
                        res = ArticleFacade.GetArticleColumnJsonStr(articleurllist, 91);
                        break;
                    default:
                        break;
                }
