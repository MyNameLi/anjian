...
public partial class Admin_IdolNew_changeDataBase : System.Web.UI.Page
{
            ...
            {               
                switch (act)
                {
                    case "store":
                        ...
                        StoreNews(posturl.ToString(), sitelist);
                        res = "{\"Success\":1}";
                        break;
                    case "innitwebsite":
    ...
    protected void StoreNews(string url, string sitelist)
    {
        ChangeDataBase(url, sitelist);
    }

    protected bool ChangeDataBase(string docref, string database)
    {
        try
        {
            ...
            drechangemeta.NewValue = database;
            cnn.Execute(drechangemeta);
            //int type = 1;
            //int userid = Convert.ToInt32(UserFacade.GetUserId());
            //ArchiveInfoFacade.AddFromIdol(docref, type, userid);
            return true;
        }
        catch
        {
