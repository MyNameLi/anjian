...
public partial class Admin_New_sgxxList : System.Web.UI.Page
{
    ...
    protected void InnitSelect()
    {
        ...
        IList<COLUMNDEFEntity> list = WebDao.Find(" ID>0 AND COLUMNSTATUS=1 AND PARENTID=200 AND SITEID=" + siteid + " order by COLUMNORDER", null);
        ListItem item = new ListItem("请选择栏目", "-1");
        SelectClomun.Items.Add(item);
        ColumnID.Items.Add(item);
        InnitSelectChild(list, 200, 0);
    }
    ...
    protected void PageDataBind()
    {
        ...
        if (clumnid != "-1")
        {
            ...
            strWhere = strWhere + " AND COLUMNID IN (SELECT ID FROM COLUMNDEF WHERE PARENTID=200)";
        }
