...
listTable.Remove = function(id,msg,act,url)
{
	if(!act)
	{
		act = "remove";	
...
listTable.EditStatus = function(obj, id, act, url) {
    ...
    $.post(url,
        { "act": act, "ajaxString": 1, "idList": id, "status": poststatus },
        function(data) {
            if (data.Error == "1") {
...
listTable.InnitAdd = function(value_list) {
    var type_list = "valueList";
    if(value_list){
        type_list = value_list;
    }
    $("*[pid='" + type_list + "']").each(function() {
        $(this).val("");
    });
    $("#back_msg").empty();
    $("#btn_add").show();
    $("#btn_reset").show();
    $("#btn_edit").hide();
    listTable.ShowEditFrame();
}

listTable.initEdit = function(id, Pk_name, back_msg, act,tag, url) {
...
listTable.ShowEditFrame = function() {
...
listTable.UpLoadFile = function(file_obj,dis_obj) {
