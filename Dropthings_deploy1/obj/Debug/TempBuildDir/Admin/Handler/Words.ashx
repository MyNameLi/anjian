    ...
    public void ProcessRequest (HttpContext context) {
        ...
        { 
            case "add":
                ...
                try
                {
                    int termID = SensitiveWordFacade.AddEntity(entity);
                    if (termID > 0)
            ...
            case "edit":
                try
                {
                    SensitiveWordFacade.UpdateSet(System.Convert.ToInt32(ID), "TERMSTR", termValue);
            ...
            case "delete":
                try
                {
                    SensitiveWordFacade.Delete(System.Convert.ToInt32(ID));
                    JsonStr.Append("\"successCode\":1");
                }
                catch { }
                break;
            case "init":
                IList<SENSITIVEWORDSEntity> initentityList = SensitiveWordFacade.GetList("");
                ...
                break;
            case "initMenu":
                IList<SENSITIVEWORDSEntity> entityList = SensitiveWordFacade.GetList("");
                ...
                foreach (SENSITIVEWORDSEntity lentity in entityList)
                {
                    ...
                    {
                        HtmlStr.AppendFormat("<li class=\"left-center-nav-ul\"><img src=\"images/yqzt_34.jpg\"/>&nbsp;&nbsp;<span><a href=\"javascript:void(null)\" id=\"childmenulist_{0}\">{1}</a></span>", lentity.ID, lentity.TERMSTR);
