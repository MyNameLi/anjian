...
var IdolNew = _idolnew.property = {
    ...
    queryParams: { "action": "query", "display_style": 6, "text": "*", "sort": "Relevance",
        "printfields": "DRETITLE,MYSITENAME,DREDATE,DOMAINSITENAME,CONTURL,C1,SAMENUM", "predict": "true"
    },
    ArchiveRefs: null,
    currPage: 1,
    Innit: function () {
        ...
        if (info_site != "all") {
            if (infotype != "all") {
                IdolNew.queryParams["fieldtext"] = "MATCH{" + infotype + "}:C1+AND+MATCH{" + info_site + "}:TASKID";
            } else {
        ...
        Lpager.OtherFn = function (totalcount) {
            IdolNew.currPage = this.currpage;
                    ...
                    if (data) {
                        if (data.Success == 1) {
                            ...
                            for (var item in data) {
                                $("#all_data_list").find("td[name='column_article_list'][pid='" + item + "']").empty().html(unescape(data[item])).parent("tr").find("td").addClass("dingzhicss");
                                //.attr("bgcolor", "#efefdf"); //.find("input[type='checkbox']").attr("disabled", "disabled");
        ...
        Lpager.LoadData(IdolNew.currPage, IdolNew.queryParams);
    },
    ...
    InnitClickFn: function () {
        ...
        $("#btn_store").click(function () {
            ...
            var urls = IdolNew.ArchiveRefs.split('+');
            var i = 0;
            _guidang(i);

            function _guidang(i) {
                $.post(IdolNew.postUrl,
                { "act": "store", "ajaxString": 1, "url": urls[i], "siteidlist": site_list.join(","), "columnid": columnid },
                    function (data) {
                        if (data.Success == 1) {
                            $("#gdjd_lb").html((i + 1) + "/" + (urls.length));
                            i = i + 1;
                            if (i < urls.length) {
                                _guidang(i);
                            } else {
