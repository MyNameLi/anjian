$(document).ready(function() {
    IdolNew.queryParams["database"] = "Safety+portalsafety";
    IdolNew.Innit();
...
var IdolNew = _idolnew.property = {
    ...
    Innit: function() {
        ...
        Lpager.OtherFn = function(totalcount) {
            ...
            $("a[name='idol_news_store']").click(function() {
                ...
                $.post(IdolNew.postUrl,
                    { "act": "store", "ajaxString": 1, "url": href, "siteidlist": "PortalSafety" },
                    function(data) {
                        if (data.Success == 1) {
                            $(current_obj).parent("td").parent("tr").css("font-weight","900");
                        }
                        if (data.Error == 1) {
        ...
        Lpager.DisFn = function(urllist) {
            $.post(IdolNew.postUrl,
                { "act": "initstorenews", "ajaxString": 1, "url_list": escape(urllist) },
                function(data) {
                    if (data) {
                        if (data.Success == 1) {
                            delete data["Success"];
                            for (var item in data) {
                                $("#all_data_list").find("input[value='" + item + "']").parent("td").parent("tr").css("font-weight", "900");
                            }                            
                        }
                    }
                },
    ...
    InnitClickFn: function() {
        ...
        IdolNew.InitCategory("category_id", 691, true);
