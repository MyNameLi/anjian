...
{
    public abstract class BriefReportTemplatePageBase : IHttpHandler, IRequiresSessionState
    {
        ...
        public string TemplateEncoding = string.Empty;
        ...
        {
            TemplateEncoding = context.Request.Form["template_encoding"];            
            ...
            try
            {
                ...
                {
                    this.LoadCurrentTemplate(labledict, TemplateEncoding);
                }
                else
                {
                ...
                string l_path = "~/Admin/reportcontent/";
                string path = this.Server.MapPath(l_path);
                string outpath = l_path.Replace("~", labledict["{#rootpath}"]);
                ...
                if (!string.IsNullOrEmpty(TemplateEncoding))
                {
                    this.Document.RenderTo(path, System.Text.Encoding.GetEncoding(TemplateEncoding));
                }
                else {
                ...
                if (!string.IsNullOrEmpty(TemplateEncoding))
                {
                    filestr = EncodeByEscape.GetEscapeStr(FileManage.ReadStr(path, TemplateEncoding));
                }
                else {
                    filestr = EncodeByEscape.GetEscapeStr(FileManage.ReadStr(path));
                }
        ...
        /// 装载当前页面的模板文档
        /// </summary>
        public virtual void LoadCurrentTemplate(Dictionary<string, string> labledict, string encoding)
        {
            if (!string.IsNullOrEmpty(this.TestType))
            {
                string filepath = this.Server.MapPath(this.TestType);
                this.LoadTemplateFile(filepath, labledict, encoding);
            }
            else
            {
                ...
                this.LoadTemplateFile(this.Server.MapPath("../reporttemplate/" + path + ".vm"), labledict, encoding);
            }
        }
        ...
        /// </summary>
        /// <param name="fileName"></param>
        protected virtual void LoadTemplateFile(string fileName, Dictionary<string, string> labledict, string encoding)
        {
            this.Document = null;
            /*if ("cache".Equals(this.TestType, StringComparison.InvariantCultureIgnoreCase) || this.IsLoadCacheTemplate)
            {
            ...
            {*/
            //测试实例模板文档
            this.Document = new TemplateDocument(fileName, Encoding.GetEncoding(encoding), this.DocumentConfig);
            /*}*/
        }
