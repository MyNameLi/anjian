...
namespace Dropthings.Data
{
    public class CompiledQueries
    {
        ...
        public class WidgetQueries
        {
            ...
            public static readonly Func<DropthingsDataContext, int, IQueryable<Widget>> GetInstanceWidgetsByTabId =
                Compile<int, Widget>((dc, TabId) =>
                    /*from Tab in dc.Tabs
                    ...
                    select widgetInstance*/
                    from wi in dc.WidgetInstances.Include("Widget")
                    from Tab in dc.Tabs
                    from Widget in dc.Widgets
                    from column in Tab.Column
                    where wi.Widget.ID == Widget.ID && TabId == Tab.ID && column.WidgetZone.ID == wi.WidgetZone.ID
                    select wi.Widget
                );
        ...
        public class TabQueries
        {
            ...
            //public static readonly Func<DropthingsDataContext, int, IQueryable<Tab>> GetTabByWidgetInstancesId =
            //   Compile<int, Tab>((dc, wiId) =>
            //       from Tab in dc.Tabs
            //       from column in Tab.Column
            //       from widgetInstance in column.WidgetZone.WidgetInstance
            //       where widgetInstance.Id == wiId
            //       select Tab
            //   );
